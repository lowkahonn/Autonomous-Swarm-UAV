...
def setup_all_uavs(set_mode_client, arming_client):
	...
	if not current_state1.armed and not current_state2.armed and not current_state3.armed and not current_state4.armed and not current_state5.armed:
	   """arming_client1(True)
	   arming_client2(True)
	   arming_client3(True)
	   arming_client4(True)
	...
	while not rospy.is_shutdown():
		...
		if shape:
                    if shape == 'circle':
                        ...
                        pose = shapes.circle(values['location'], values['pose'], values['direction'], values['numuav'], values['offset'])
		numuav = current_num_uav
		...
		if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_o:
                        pose = shapes.circle(location, pose, direction, numuav, offset)
                        shape = 'circle'

                    if event.key == pygame.K_x:
                        pose = shapes.xform(location, pose, direction, numuav, offset)
                        shape = 'xform'

                    if event.key == pygame.K_t:
                        pose = shapes.triangle(location, pose, direction, numuav, offset)
                        shape = 'triangle'

                    if event.key == pygame.K_l:
                        pose = shapes.line(location, pose, direction, numuav, offset)
                        shape = 'line'
